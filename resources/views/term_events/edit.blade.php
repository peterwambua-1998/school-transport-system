    ...
    <div class="col-lg-12 grid-margin stretch-card">
        <div class="card">
            <div class="card-body">
                ...
                <hr>
                <form action="{{ route('term_events.update', $term->id) }}"  method="POST" id="my-form">
                    @csrf
                    @method('PATCH')
                    ...
                    <div class="row">
                        ...
                        <div class="mb-3 col-md-6 col-sm-12">
                            ...
                            <select name="within" id="within" class="form-select">
                                ...
                                <option @if ($term->within == 'yes')
                                    selected
                                @endif value="yes">Yes</option>
                                <option @if ($term->within == 'no')
                                    selected
                    ...
                    <div class="row">
                        ...
                        <div class="mb-3 col-md-6">
                            ...
                            <input type="date" name="ends" class="form-control" id="date_end" required value="{{ $term->ends }}">
                    ...
                    <div class="row">
                        <div class="mb-3 col-md-6 col-sm-12">
                            ...
                            <input type="time" name="start_time" class="form-control" id="start_time" required value="{{ $term->start_time }}">
                            <span class="issue" id="start_time_error"></span>
                        </div>
                        <div class="mb-3 col-md-6 col-sm-12">
                            <label class="form-label" for="platenum">End Time</label>
                            <input type="time" name="end_time" class="form-control" id="end_time" required value="{{ $term->end_time }}">
                    ...
                    <div class="row">
                        <div class="col-md-6 mb-3">
                            ...
                            <input placeholder="location" type="text" name="location" value="{{$term->location}}" class="form-control" id="location" required>
                            <span class="issue" id="location_error"></span>
                        
                        </div>
                        <div class="col-md-6">
                            <label  class="form-label" for="inputState">Grades</label>
                            <select id="grades" class="js-example-basic form-select" multiple="mutiple" name="grades[]" required>
                               
                                <option>select...</option>
                                @foreach ($grades as $grade)
                                    @php
                                    $event_grades = DB::table('event_grades')->where('grade','=', $grade->id)->first();
                                    @endphp
                                    <option @if($event_grades) @if($event_grades->grade== $grade->id) selected @endif @endif value="{{ $grade->id }}">{{ $grade->name }}</option>
                                @endforeach
                            </select>
                    ...
                    <div class="row">
                        <div class="col-md-6 mb-3" id="transport" style="display: none">
                            <label class="form-label" for="title">Transport</label>
                            <select name="transport" id="transport" class="form-control">
                                <option>select...</option>
                                <option @if ($term->transport == "school")
                                    selected
                                @endif value="school">School</option>
                                <option @if ($term->transport == "parent")
                                    selected
                                @endif value="parent">Parent</option>
                            </select>
                    ...
                    <div class="text-center">
                        <button type="button" id="submit-btn" class="btn btn-success mt-3"><ion-icon style="position: relative; top:2px; right: 5px; color: #fff;" name="save"></ion-icon> Update Event</button>
                    </div>
                </form>
            </div>
        </div>
...
<script defer>
    $(document).ready( function () {
        ...
        let within = $('#within').find(':selected').val();
