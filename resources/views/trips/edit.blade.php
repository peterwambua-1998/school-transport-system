...
<div class="row">
    <div class="col-lg-12 grid-margin stretch-card">
        <div class="card">
            <div class="card-body">
                <h4 class="card-title">Edit Trip For Vehicle {{$vehicle->plate_num}}</h4>
                <hr>
                <form action="{{ route('trips.update', $trip->id) }}" method="POST" id="my-form">
                    @csrf 
                    @method('PATCH')
                    <div class="row">
                        <div class="mb-3 col-md-6">
                            <label class="form-label" for="title">Trip Title</label>
                            <input type="text" name="title" class="form-control" id="title" placeholder="Trip Title" required value="{{$trip->title}}">
                        ...
                        <div class="mb-3 col-md-6">
                            ...
                            <select id="am_pm" class="form-control" name="route_time" required>
                                ...
                                <option @if ($trip->time == 'am')
                                    selected
                                @endif value="am">am</option>
                                <option @if ($trip->time == 'pm')
                                    selected
                                @endif value="pm">pm</option>
                    ...
                    <div class="row">
                        <div class="mb-3 col-md-6">
                            ...
                            <input type="time" name="from" class="form-control" id="from" placeholder="from" required value="{{$trip->time_from}}">
                            <span class="issue" id="from_error"></span>
                        </div>

                        <div class="mb-3 col-md-6">
                            <label class="form-label" for="platenum">To</label>
                            <input type="time" name="to" class="form-control" id="to" placeholder="to" required value="{{$trip->time_to}}">
                    ...
                    <div class="row">
                        <div class="col-md-6 mb-3">
                            <label class="form-label">{{ucfirst($tr->plural) ?? 'Grades'}}</label>
                            @if (count($grades) <= 0)
                            ...
                            <select id="grades" class="js-example-basic-multiple form-select" name="grades[]" multiple="multiple" data-width="100%">
                                ...
                                @php
                                    $trip_grade = DB::table('grade_groups')->where('trip_id','=', $trip->id)->where('grade_id','=', $grade->id)->first();
                                @endphp
                                    <option @if ($trip_grade)
                                        @if ($trip_grade->grade_id == $grade->id)
                                            selected
                                        @endif
                        ...
                        <div class="col-md-6 mb-3">
                            ...
                            <select id="route" class="form-select" name="route" data-width="100%">
                                ...
                                @foreach ($vehicle_routes as $vehicle_route)
                                    <option @if ($trip->route_id == $vehicle_route->id)
                                        selected
                                    @endif value="{{$vehicle_route->route_id}}">{{App\Models\Route::where('id',$vehicle_route->route_id)->first()->title}}</option>
                                @endforeach
                            </select>
                            @endif
                    ...
                    <div class="text-center">
                        <button type="button" id="submit-btn" class="btn btn-success mt-3"><ion-icon style="position: relative; top:2px; right: 5px; color: #fff;" name="save"></ion-icon> Update Trip</button>
                    </div>
