    ...
    <table class="table table-bordered table-striped" id="dataTableExample2">
        ...
        <tbody>
            ...
            <tr>
                ...
                <td>{{Str::limit($daily->comment, 20)}}</td>
                <td>
                    @if ($daily->images->isNotEmpty())
                        <i class="fa-solid fa-image shows-off-routine" data-ids="{{$daily->id}}"></i>
                    @endif
                    @if ($daily->video)
...
@push('custom-scripts')
    <script defer>
        $(function() {
            var nodeList = $('.shows-off-routine');
