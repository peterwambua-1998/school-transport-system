...
<div class="table-responsive">
    <h5 class="card-title">Incidents caused by students</h5>
    <table class="table table-bordered table-striped"  id="dataTableExample" data-ordering="false">
        ...
        <tbody>
            <?php $number = 1; ?>
            @foreach ($incidentStds as $incidentAt)
                
                <tr>
                    <td>{{ $number }}</td>
                    <?php $number++; ?>
                    <td>
                        <?PHP $st= App\Models\Student::where('id','=',$incidentAt->student_assulter)->first();?>
                        {{
                            $st->last_name
                        }}
                    </td>
                    ...
                    <td>{{App\Models\Trip::where('id','=',$incidentAt->trip)->first()->title}} {{App\Models\Trip::where('id','=',$incidentAt->trip)->first()->time}}</td>
        ...
        <div class="modal-body">
            <ul class="list-group mb-3">
                ...
                <li class="list-group-item">
                    <?php $st= App\Models\Student::where('id','=',$incidentAt->student_assulter)->first() ?>
                    <span class="ml-5 text-muted">Student Name:</span> <span>{{$st->first_name}} {{$st->last_name}}</span>
                </li>
